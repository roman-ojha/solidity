...
contract local {
    ...
    function store() public pure returns (uint256) {
        ...
        // string name="Roman";
        // NOTE: we can declare string variable on local label it only can be able to declare on contract label if we can to use string variable on local label the we have to use 'memory' keyword
        string memory name = "Roman";
