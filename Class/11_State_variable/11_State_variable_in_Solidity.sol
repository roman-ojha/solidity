...
contract State {
    ...
    // if we will write public in declaration of variable the for that particular variable function will automatically been created
    // NOTE: if we have declare state variable the we can change the value of that variable in the contract block, but we can create constructor and assign value to that variable, or we can create setter function
    constructor() {
        age = 10;
    }
    ...
    /*
        *) Properties of state variable:
            ...
            -> on cpp programming language, if we declare variable it will automatically assign NaN as a value, but in a case of solidity, value will not assign as NaN, null etc.., for int it will assign as 0(zero) value
